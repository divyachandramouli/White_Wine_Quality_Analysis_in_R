...
It would be helpful to create a discrete quality variable
```{r}
...
Let's view the dataframe with the quality buckets to make sure it is right
```{r}
summary(wq$quality.bucket)
```
Distribution of wines by quality bucket

```{r}
qplot(quality.bucket, data=wq)
```
...
```
Let's create a residual.sugar.bucket variable
...
Let's look at the distribution of wines by alcohol bucket

```{r}
qplot(alcohol.bucket, data=wq)
```
...
Let's look at the distribution of wines by density bucket

```{r}
qplot(density.bucket, data=wq)
```
...
Univariate explorationhelped get an idea about the individual distributions of data and it was interesting to apply transformations  and see their effect.  
...
Thelinear predictive model for quality was built using alcohol, density, chlorides 
