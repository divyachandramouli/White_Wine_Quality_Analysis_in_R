...
Let's examine the variables and the kind of data they hold
...
Let's check for missing or null values in the dataset
...
Are here duplicate records in the dataset? 
...
```
 ...
 Now, Let's study each variable in detail
...
```
 ...
 The distribution of fixed acidity looks like the 
 bell curve (close to normal distribution). The range of fixed acidity is from
 3.8 to 14.2 from the summary table. In the plot we can clearly see that the 
 fixed acidity values for the bulk of wines lies between 4 and 10 with outliers 
 on the right. 6.8 is the most commonly occurring value of fixed acidity 
