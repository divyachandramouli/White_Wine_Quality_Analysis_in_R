...
be useful to go into the Pearson coefficient values for the variables excluding 
quality
...
```{r}
ggplot(aes(x=density, y=alcohol), data=wq)+geom_point()
```

Let's reduce overplotting and look at the plot again with a linear trendline
```{r}
ggplot(aes(x=density, y=alcohol), data=wq)+geom_point(alpha=1/20,position='jitter')+
  geom_smooth(method = 'lm')+scale_y_continuous(limits=c(7,15))
...
The blue dot marks the mean density of each quality level of wine and we see that
higher quality wines have lesser density

I am curious to see how alcohol level impacts quality

The summary statistics for alcohol levels:
...
Let's view the dataframe with the alcohol buckets to make sure it is right
```{r}
...
```{r}
ggplot(aes(x=factor(quality), y=alcohol), data=wq)+geom_boxplot(fill='green')+stat_summary(fun.y = mean, color='black',geom='point', size=2)
```
